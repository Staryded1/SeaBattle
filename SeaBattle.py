...
class Dot:
    def __init__(self, coor_x, coor_y):
        self.coor_x = coor_x  # Координата x
        self.coor_y = coor_y  # Координата y
...
class Ship:
    ...
    def get_dots(self):
        # Метод для получения всех точек корабля в зависимости от его направления и размера
        dots = []
...
class Board:
    ...
    def win_check(self):
        # Проверка на победу: если количество уничтоженных кораблей равно общему количеству кораблей
    ...
    def __str__(self):
        field = "   | 1 | 2 | 3 | 4 | 5 | 6 |\n"
        field += "-" * 31 + "\n"  # Линия под заголовком столбцов

        for i, row in enumerate(self.board):
            field += f"{i+1:2} | " + " | ".join(row) + " |\n"
            field += "-" * 31 + "\n"  # Линия после каждой строки поля
        ...
        for ship in self.ships_list:
            if ship.shooted(dot):
                ...
                if ship.health_count == 0:
                    ...
                    return True
        self.board[dot.coor_x][dot.coor_y] = "Т" 
        print("Мимо!")
        return False

    def again(self):
        # Очистка списка занятых точек для новой игры
        self.busy_dots = []
...
class Game:
    ...
    def try_board(self):
        # Генерация случайного расположения кораблей на игровом поле
        ships_len = [3, 2, 2, 1, 1, 1, 1]  # Размеры кораблей
        attempts = 0
